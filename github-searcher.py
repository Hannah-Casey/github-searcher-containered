...
import requests
import logging
OUTPUT_FOLDER = '/var/log/output/'
LOG_FILE = os.path.join(OUTPUT_FOLDER, 'example.log')
print(LOG_FILE)
...
args.database = os.path.join(OUTPUT_FOLDER, args.database)
args.statistics = os.path.join(OUTPUT_FOLDER, args.statistics)
...
logging.debug('                 ┌────────────┬────────────┐')
logging.debug('                 │ population │   sample   │')
logging.debug('                 ├────────────┼────────────┤')
...
def print_stratum(overwrite=False):
    ...
    logging.debug('{} │ {} │ {} │ {}'.format(size, pop_str, sam_str, per))
...
def print_footer():
    ...
    logging.debug('                 └────────────┴────────────┘')
    logging.debug('{}   {}   {}  {}'.format(size, pop_str, sam_str, per))
    logging.debug('                   (estimated)') if est_pop > -1 else logging.debug('')
    logging.debug('')
...
def download_files_from_page(res):
    ...
    for item in res.json()['items']:
        if not known_file(item):
            repo = item['ch.tuw.githubsearcher.repository']
            infos = get(repo['url']).json()
            license = infos['license']
            if(license != None):
                license = license['name']
            insert_repo(repo, license)         
            try:
            ...
            try:
                ...
                #Get all commits from that ch.tuw.githubsearcher.repository
                commits = get(repo['commits_url'].replace('{/sha}', '')).json()
                for i in range(len(commits)):
                    commit = get(commits[i]['url']).json()
                    commit_id = (insert_git_commit(commit, repo['id']))
                    #Get all the files with changes
                    for c in commit['files']:
                        commit_contents = get(c['contents_url']).json()
                        insert_commit_content(commit_contents, commit_id, c)
            except:
                continue
        sam += 1
        total_sam += 1
...
def insert_git_commit(git_commit, repo_id):
    db.execute('''
        INSERT OR IGNORE INTO git_commit
            (message, author, url, total, additions, deletions, sha, repo_id)
        VALUES(?,?,?,?,?,?,?,?)
        ''', 
        (git_commit['commit']['message']
        , git_commit['commit']['author']['name']
        , git_commit['url']
        , git_commit['stats']['total']
        , git_commit['stats']['additions']
        , git_commit['stats']['deletions']
        , git_commit['sha']
        , repo_id
        ))
    db.commit()  
...
def insert_commit_content(commit_content, commit_id, commit_file):
    db.execute('''
        INSERT OR IGNORE INTO commit_content
            (file_name, sha, status, additions, deletions, changes, contents_url, content, commit_id)
            VALUES(?,?,?,?,?,?,?,?,?)
        ''',
        ...
        , commit_file['changes']
        , commit_file['contents_url']
        , base64.b64decode(commit_content['content']).decode('UTF-8')
        , commit_id
        ))
    db.commit()     
...
def known_file(item):
    cur = db.execute("select count(*) from file where path = ? and repo_id = ?", 
        (item['path'], item['ch.tuw.githubsearcher.repository']['id']))
